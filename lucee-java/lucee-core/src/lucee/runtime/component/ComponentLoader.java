...
import lucee.runtime.ComponentPage;
...
public class ComponentLoader {
    ...
    private static Object load(PageContext pc,PageSource child,String rawPath, Boolean searchLocal, Boolean searchRoot, Map interfaceUDFs, boolean returnPage) throws PageException  {
    	...
    	boolean isRelPath=!StringUtil.startsWith(pathWithCFC,'/');
    	...
    	if(!ArrayUtil.isEmpty(acm)) {
        	...
        	for(int y=0;y<acm.length;y++){
	    		...
	    		if(page!=null)	{
	    			return returnPage?page:load(pc,page,page.getPageSource(),trim(path.replace('/', '.')),isRelPath,interfaceUDFs);
	        	}
        	}
    	}
    ...
    // CACHE
	    ...
	    if(searchLocal && isRelPath){
	    	...
	    	if(doCache){
	    		...
	    		if(page!=null) return returnPage?page:load(pc,page,page.getPageSource(),trim(path.replace('/', '.')),isRelPath,interfaceUDFs);
	    	}
	    }
	    
    	// check import cache
    	if(doCache && isRelPath){
    ...
    // SEARCH
    	...
    	if(StringUtil.startsWithIgnoreCase(rawPath, "cfide.")) {
    		String rpm="org.lucee.cfml."+rawPath.substring(6);
    		try{
