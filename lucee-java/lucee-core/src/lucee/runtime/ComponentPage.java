...
public abstract class ComponentPage extends PagePlus  {
	...
	public void call(PageContext pc) throws PageException {
		...
		try {
            ...
            //if(path.size()>1 ) {
            if(path.size()>1 && !(path.size()==3 && ListUtil.last(path.getE(2).toString(),"/\\",true).equalsIgnoreCase(lucee.runtime.config.Constants.APP_CFC)) ) {// MUSTMUST bad impl -> check with and without application.cfc
