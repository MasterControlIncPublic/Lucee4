...
public final class Admin extends TagImpl implements DynamicAttributes {
	...
	public static final String[] ORM_JARS = new String[]{"antlr.jar","dom4j.jar","hibernate.jar","javassist.jar","jta.jar","slf4j-api.jar","lucee-sl4j.jar"};
	...
	public static final String[] UPDATE_JARS = new String[]{"lucee-inst.jar","ehcache.jar","antlr.jar","dom4j.jar","hibernate.jar","javassist.jar","jta.jar","slf4j-api.jar","lucee-sl4j.jar","metadata-extractor.jar","icepdf-core.jar","com.naryx.tagfusion.cfx.jar"};
	...
	*/
    ...
    private void doUpdatePlugin() throws PageException, IOException {
        ...
        if(!PluginFilter.doAccept(srcDir))
        	throw new ApplicationException("plugin ["+strSrc+"] is invalid, missing one of the following files [Action.cfc,language.xml] in root, existing files are ["+lucee.runtime.type.util.ListUtil.arrayToList(srcDir.list(), ", ")+"]");
    ...
    private void doGetJavaCFXTags() throws PageException {
        ...
        lucee.runtime.type.Query qry=new QueryImpl(new Collection.Key[]{
    ...
    private void doGetCFXTags() throws PageException {
        Map map = config.getCFXTagPool().getClasses();
        lucee.runtime.type.Query qry=new QueryImpl(new String[]{"displayname","sourcename","readonly","isvalid","name","procedure_class","keep_alive"},map.size(),"query");
        Iterator it = map.keySet().iterator();
    ...
    private void doGetExtensions() throws PageException {
    	...
    	lucee.runtime.type.Query qry=new QueryImpl(new String[]{
    ...
    private void doGetRunningThreads() throws PageException {
    	lucee.runtime.type.Query qry=new QueryImpl(new String[]{"Id","Start","Timeout","ThreadType","StackTrace","TagContext",
    ...
    private void doGetJars() throws PageException {
		...
		lucee.runtime.type.Query qry=new QueryImpl(new String[]{"name","source"},new String[]{"varchar","varchar"},0,"jars");
    ...
    private void doVerifyRemoteClient() throws PageException {
        ...
        if(sn.getVersion()==SerialNumber.VERSION_COMMUNITY)
            throw new SecurityException("can not verify remote client with "+sn.getStringVersion()+" version of lucee");
	    */
    	ProxyData pd=null;
	...
	private void doGetCacheConnections() throws PageException  {
		...
		lucee.runtime.type.Query qry=new QueryImpl(new String[]{"class","name","custom","default","readOnly","storage"}, 0, "connections");
        Map.Entry entry;
        CacheConnection cc;
    ...
    private void doGetDatasources() throws PageException {
        ...
        Map ds = config.getDataSourcesAsMap();
        Iterator it = ds.keySet().iterator();
        lucee.runtime.type.Query qry=new QueryImpl(new String[]{"name","host","classname","dsn","DsnTranslated","database","port",
