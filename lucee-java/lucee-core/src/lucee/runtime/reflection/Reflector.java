...
public final class Reflector {
        ...
        for(int i=0;i<classes.length;i++){
        	if(args[i] instanceof JavaObject) {
        		...
        		throw new NoSuchMethodException("The "+pos(i+1)+" parameter of "+methodName+"("+getDspMethods(classes)+") ia a object created " +
        				...
        				"that takes zero arguments. Lucee cannot instantiate it for you, please use the .init(...) method to instantiate it with the correct parameters first");
	...
	/*private static void checkAccesibilityx(Object obj, Key methodName) {
		if(methodName.equals(SET_ACCESSIBLE) && obj instanceof Member) {
        	...
        	Class<?> cls = member.getDeclaringClass();
        	if(cls.getPackage().getName().startsWith("lucee.")) {
        		throw new PageRuntimeException(new SecurityException("Changing the accesibility of an object's members in the lucee.* package is not allowed"));
        	}        	
        }
	}*/
