...
public final class ConfigWebFactory extends ConfigFactory {
	...
	private static void loadResourceProvider(ConfigServerImpl configServer, ConfigImpl config, Document doc) throws ClassException {
		...
		if (providers != null && providers.length > 0) {
			...
			String s3Class = "lucee.commons.io.res.type.s3.S3ResourceProvider";
	...
	private static String deRailo(String className) {
		if(className==null) return null;
		className=className.trim();
		if(className.startsWith("railo.commons.") || className.startsWith("railo.runtime.") || className.startsWith("railo.cfx."))
			return "lucee"+className.substring(5);
		return className;
	}
	...
	private static void loadDumpWriter(ConfigServerImpl configServer, ConfigImpl config, Document doc) throws ClassException {
		...
		if (writers != null && writers.length > 0) {
			...
			for (int i = 0; i < writers.length; i++) {
				strClass = deRailo(writers[i].getAttribute("class"));
	...
	private static void loadListener(ConfigServerImpl configServer, ConfigImpl config, Document doc) {
		if (config instanceof ConfigServer) {
			...
			String strClass = deRailo(listener.getAttribute("class"));
	...
	private static void loadSecurity(ConfigServerImpl configServer, ConfigImpl config, Document doc) {
		...
		if (config instanceof ConfigServer) {
			Element luceeConfiguration = doc.getDocumentElement();
			String serial = luceeConfiguration.getAttribute("serial-number");
	...
	public static void createContextFiles(Resource configDir, ServletConfig servletConfig, boolean doNew) throws IOException {
		...
		f = contextDir.getRealResource("lucee-applet.cfm");
		if (!f.exists())
			createFileFromResourceEL("/resource/context/lucee-applet.cfm", f);
	...
	private static void loadMappings(ConfigServerImpl configServer, ConfigImpl config, Document doc, int mode) throws IOException {
		...
		if (hasAccess) {
			boolean hasLuceeServerContext=false;
	...
	private static void loadFlex(ConfigServerImpl configServer, ConfigImpl config, Document doc) {
		...
		if (StringUtil.isEmpty(strCaster))
			strCaster = deRailo(el.getAttribute("caster-class"));

		// arguments
		String strArgs = el.getAttribute("caster-arguments");
		if (StringUtil.isEmpty(strArgs))
			strArgs = deRailo(el.getAttribute("caster-class-arguments"));
	...
	private static void loadExeLog(ConfigServerImpl configServer, ConfigImpl config, Document doc) {
		...
		// class
		String strClass = deRailo(el.getAttribute("class"));
		Class clazz;
		if (!StringUtil.isEmpty(strClass)) {
			try {
	...
	private static void loadDataSources(ConfigServerImpl configServer, ConfigImpl config, Document doc) throws ClassException {
		...
		for (int i = 0; i < accessCount; i++) {
			...
			if (dataSource.hasAttribute("database")) {
				setDatasourceEL(config, datasources, dataSource.getAttribute("name"), deRailo(dataSource.getAttribute("class")), dataSource.getAttribute("host"),
	 ...
	 * @throws NoSuchAlgorithmException 
	 */
	private static void loadLuceeConfig(ConfigServerImpl configServer, ConfigImpl config, Document doc) throws IOException  {
		Element luceeConfiguration = doc.getDocumentElement();
		
		// salt (every context need to have a salt)
		String salt=luceeConfiguration.getAttribute("salt");
		...
		// password
		Password pw=Password.getInstance(luceeConfiguration,salt,false);
		if(pw!=null) 
		...
		if(config instanceof ConfigServerImpl) {
			...
			String keyList=luceeConfiguration.getAttribute("auth-keys");
		...
		// api key
		String apiKey = luceeConfiguration.getAttribute("api-key");
		...
		// default password
		if (config instanceof ConfigServerImpl) {
			pw=Password.getInstance(luceeConfiguration,salt,true);
			if(pw!=null)
				((ConfigServerImpl) config).setDefaultPassword(pw);
		}

		// mode
		String mode = luceeConfiguration.getAttribute("mode");
		...
		String cFc = luceeConfiguration.getAttribute("check-for-changes");
		...
		
		{
			...
			if (tags != null) {
				for (int i = 0; i < tags.length; i++) {
					...
					c = deRailo(tag.getAttribute("class"));
	...
	private static void loadVideo(ConfigServerImpl configServer, ConfigImpl config, Document doc) throws ApplicationException {
		...
		// video-executer
		if (video != null) {
			str = deRailo(video.getAttribute("video-executer-class"));
			if (StringUtil.isEmpty(str))
	...
	private static void loadORM(ConfigServer configServer, ConfigImpl config, Document doc) {
		...
		// engine
		String defaultEngineClass = "lucee.runtime.orm.hibernate.HibernateORMEngine";

		// print.o("orm:"+defaulrEngineClass);
		String strEngine = null;
		if (orm != null)
			strEngine = deRailo(orm.getAttribute("engine-class"));
	...
	private static void loadScope(ConfigServerImpl configServer, ConfigImpl config, Document doc, int mode) throws PageException {
		...
		// Cluster Scope
		if (!hasCS) {
			String strClass = deRailo(scope.getAttribute("cluster-class"));
	...
	private static void loadMonitors(ConfigServerImpl configServer, ConfigImpl config, Document doc) throws IOException {
		...
		for (int i = 0; i < children.length; i++) {
			...
			className = deRailo(el.getAttribute("class"));
	...
	private static void loadSearch(ConfigServer configServer, ConfigImpl config, Document doc) throws PageException {
		...
		String strEngineClass = deRailo(search.getAttribute("engine-class"));
		...
		if (se == null)
			se = new lucee.runtime.search.lucene2.LuceneSearchEngine();
	...
	private static void loadExtensions(ConfigServerImpl configServer, ConfigImpl config, Document doc) {
		...
		for (int i = 0; i < ConfigImpl.LUCEE_EXTENSION_PROVIDERS.length; i++) {
			list.put(ConfigImpl.LUCEE_EXTENSION_PROVIDERS[i], "");
		}

		for (int i = 0; i < xmlProviders.length; i++) {
			provider = xmlProviders[i].getAttribute("url");
			if (!StringUtil.isEmpty(provider, true) && !"http://www.lucee-technologies.com/ExtensionProvider.cfc".equals(provider)
					&& !"http://www.lucee.ch/ExtensionProvider.cfc".equals(provider)) {
	...
	private static void loadApplication(ConfigServerImpl configServer, ConfigImpl config, Document doc, int mode) throws IOException, PageException {
		...
		// admin sync
		String strClass = deRailo(application.getAttribute("admin-sync-class"));
		if (StringUtil.isEmpty(strClass))
			strClass = deRailo(scope.getAttribute("admin-sync"));
		if (StringUtil.isEmpty(strClass))
			strClass = deRailo(scope.getAttribute("admin-synchronisation-class"));
		if (StringUtil.isEmpty(strClass))
			strClass = deRailo(scope.getAttribute("admin-synchronisation"));
