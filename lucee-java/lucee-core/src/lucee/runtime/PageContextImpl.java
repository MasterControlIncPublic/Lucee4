    ...
    public Application applicationScope() throws PageException {
		if(application==null) {
			if(!applicationContext.hasName())
				throw new ExpressionException("there is no application context defined for this application","you can define a application context with the tag "+lucee.runtime.config.Constants.CFAPP_NAME+"/"+lucee.runtime.config.Constants.APP_CFC);
	...
	public void handlePageException(PageException pe) {
		if(!Abort.isSilentAbort(pe)) {
			...
			if(ep!=null) {
				...
				else pe=new ApplicationException("The error page template for type request only works if the actual source file also exists. If the exception file is in an Lucee archive (lco), you need to use type exception instead.");
			}
